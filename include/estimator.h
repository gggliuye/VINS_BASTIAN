...
//////////////// OUR SOLVER //////////////////
    ...
    Vec7 vPic[NUM_OF_CAM];
    Matrix3d ric[NUM_OF_CAM];
    Vector3d tic[NUM_OF_CAM];

    //Vec7 vPose[(WINDOW_SIZE + 1)];
    ...
    void SaveMarginalizedFrameHostedPoints(myslam::backend::Problem &problem);
    void UpdateCurrentPointcloud();


    double para_Pose[WINDOW_SIZE + 1][SIZE_POSE]; // x, y, z, qx, qy, qz, qw
    double para_SpeedBias[WINDOW_SIZE + 1][SIZE_SPEEDBIAS]; // vx, vy, vz, bax, bay, baz, bgx, bgy, bgz
    //double para_Feature[NUM_OF_F][SIZE_FEATURE]; // inversedepth
    double para_Ex_Pose[NUM_OF_CAM][SIZE_POSE]; // tic : x, y, z, qx, qy, qz, qw
